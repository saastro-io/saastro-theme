...
- **Con Workers Static Assets, lo PRERENDERIZADO no invoca al Worker: ni middleware, ni sus
  cabeceras.** La petición que casa con un fichero del bundle la sirve la capa de assets sin
  llamar al Worker; la que no casa la resuelve el Worker. De ahí salen dos trampas que van al
  revés de lo que parece. **(1)** `public/_headers` **solo viste lo que sirven los assets** —
  Cloudflare lo dice literalmente: «Custom headers defined in the `_headers` file are not
  applied to responses generated by your Worker code, even if the request URL matches a rule
  defined in `_headers`» ([docs](https://developers.cloudflare.com/workers/static-assets/headers/)).
  Un site con rutas SSR (`prerender = false`, o sea las landings de `/lp/*`) las sirve **sin esas
  cabeceras**, y son justo las del tráfico de campaña: medido en un descendiente, `/` con 4
  cabeceras y `/lp/*` con 1. Hacen falta DOS listas —`_headers` para los assets y una aplicada
  desde el middleware para lo que salga del Worker— y mantenerlas a la par. **(2)** Y el
  corolario caro: si el middleware no corre en lo prerenderizado, **una redirección escrita ahí
  tampoco ocurre**. Un apex → www del middleware funciona en `/lp/*` y no en `/`. Como las
  cabeceras SÍ llegan a esas páginas (las pone `_headers`), invita a deducir «llegan las
  cabeceras, luego el middleware corre aquí»; es falso y manda a buscar el fallo dentro de un
  middleware que está perfecto. Comprobado en un descendiente el 10-sep-2026: el mismo código
  daba 301 en la ruta SSR y 200 en la prerenderizada. **`_redirects` no lo salva: no acepta el
  DOMINIO en el origen de la regla** — medido con control, una regla por RUTA en ese fichero
  devuelve 301 y una por HOST devuelve 200, así que el fichero se lee y la forma con host no
  dispara. Una redirección de dominio se resuelve con una **Redirect Rule de zona**, que corre
  antes que el Worker y antes que los assets. Verificación mínima al montar un site: `curl -sI`
  sobre `/`, una ruta SSR y un 404.
